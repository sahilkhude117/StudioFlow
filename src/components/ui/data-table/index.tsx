...
>({
  ...
  }, [currentCursor, table.getState().pagination.pageSize, router, searchParams]);
        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="hover:bg-background h-20">
            ...
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  key={row.id}
                  className={onRowClick ? 'cursor-pointer h-15' : ''}
